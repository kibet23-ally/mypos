...
export default function SASystemAnalytics() {
  ...
  const items = [
    { label: 'Total Tenants', value: stats.tenants, icon: Database, color: 'text-blue-700' },
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center gap-2"><BarChart3 className="w-5 h-5 text-blue-600"/><h1 className="text-xl font-bold text-slate-800">System Analytics</h1></div>
        ...
        items.map(s => (
          <Card key={s.label} style={CARD} className="rounded-2xl">
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center gap-2 mb-1"><s.icon className={`w-4 h-4 ${s.color}`}/><p className="text-xs text-slate-400">{s.label}</p></div>
