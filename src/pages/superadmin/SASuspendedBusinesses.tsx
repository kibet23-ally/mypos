...
const PAGE = 20; const CARD = { background: '#ffffff', borderColor: '#E2E8F0' };
...
export default function SASuspendedBusinesses() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center gap-2 mb-2"><AlertCircle className="w-5 h-5 text-red-500"/><h1 className="text-xl font-bold text-slate-800">Suspended Businesses</h1><Badge variant="destructive">{total}</Badge></div>
      <Card style={CARD}>
        ...
        <CardContent className="overflow-x-auto">
          ...
          : tenants.length===0 ? <p className="text-center py-8 text-slate-400">No suspended businesses.</p>
          : <table className="w-full text-sm whitespace-nowrap">
              <thead><tr className="border-b border-slate-100">{['Business','Email','Reason','Actions'].map(h=><th key={h} className="text-left py-2 px-3 font-semibold text-slate-600">{h}</th>)}</tr></thead>
              <tbody>{tenants.map(t=>(
                <tr key={t.id} className="border-b border-slate-50 hover:bg-red-50">
