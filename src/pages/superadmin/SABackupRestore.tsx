...
export default function SABackupRestore() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center gap-2 mb-2"><Database className="w-5 h-5 text-primary"/><h1 className="text-xl font-bold text-foreground">Backup & Restore</h1></div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {info.map(i => (
          <Card key={i.title} style={CARD} className="rounded-2xl">
            <CardContent className="pt-5 pb-4">
              <div className="flex items-center gap-3 mb-2"><div className="w-8 h-8 bg-accent rounded-xl flex items-center justify-center"><i.icon className="w-4 h-4 text-primary"/></div><h3 className="font-semibold text-foreground">{i.title}</h3></div>
      ...
      <Card style={CARD}>
        <CardContent className="pt-4 pb-4">
          <p className="text-sm text-muted-foreground">For direct database management, access the <a href="https://app.supabase.com" target="_blank" rel="noopener noreferrer" className="text-primary underline font-medium">Supabase Dashboard</a> with your project credentials.</p>
        </CardContent>
      </Card>
    </div>
  );
