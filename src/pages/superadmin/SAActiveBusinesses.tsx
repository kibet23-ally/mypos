...
export default function SAActiveBusinesses() {
  ...
  const load = useCallback(async () => {
    ...
    let q = supabase.from('tenants').select('id,business_name,plan,plan_expires_at,email,phone,created_at', { count: 'exact' })
      .eq('suspended', false).gt('plan_expires_at', now).order('created_at', { ascending: false }).range(page * PAGE, (page + 1) * PAGE - 1);
