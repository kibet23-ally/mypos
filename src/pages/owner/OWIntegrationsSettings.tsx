...
const integrations = [
  ...
  { icon: CreditCard, title: 'Card Payments', desc: 'Accept Visa/Mastercard via your POS terminal. Card transactions are recorded automatically on every sale.', status: 'built-in', color: 'text-blue-600', bg: 'bg-blue-50' },
  ...
  { icon: Truck, title: 'Delivery / Logistics', desc: 'Integration with local courier and logistics providers coming soon.', status: 'coming-soon', color: 'text-slate-400', bg: 'bg-slate-50' },
];
const STATUS_LABELS: Record<string,{label:string;cls:string}> = {
  'available':     { label: 'Available', cls: 'bg-green-100 text-green-700' },
  'built-in':      { label: 'Built-in', cls: 'bg-blue-100 text-blue-700' },
  'contact-admin': { label: 'Contact Admin', cls: 'bg-orange-100 text-orange-700' },
  'coming-soon':   { label: 'Coming Soon', cls: 'bg-slate-100 text-slate-500' },
};
export default function OWIntegrationsSettings() {
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center gap-2 mb-2"><Globe className="w-5 h-5 text-blue-600"/><h1 className="text-xl font-bold text-slate-800">Integrations</h1></div>
      <p className="text-sm text-slate-500">Connect PosifyPro with external services and payment gateways. Contact your Super Admin to enable payment or messaging integrations.</p>
