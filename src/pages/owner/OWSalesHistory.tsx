...
const PM_COLORS: Record<string,string> = { cash:'bg-green-100 text-green-700', mpesa:'bg-accent text-primary', card:'bg-purple-100 text-purple-700', bank_transfer:'bg-orange-100 text-orange-700' };
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Button onClick={exportCSV} variant="outline" className="gap-1 h-9 text-muted-foreground"><Download className="w-4 h-4" />Export CSV</Button>
      </div>

      {/* Summary row */}
      <div className="grid grid-cols-3 gap-3">
        {[{label:'Revenue (page)',val:totals.revenue,color:'text-primary'},{label:'COGS (page)',val:totals.cogs,color:'text-muted-foreground'},{label:'Gross Profit (page)',val:totals.profit,color:'text-green-700'}].map(s=>(
      ...
      <Card style={CARD}>
        ...
        <CardContent className="overflow-x-auto">
          ...
          : sales.length === 0 ? <div className="text-center py-10 text-muted-foreground"><History className="w-8 h-8 mx-auto mb-2 opacity-40"/><p>No sales found.</p></div>
          : (
            <table className="w-full text-sm whitespace-nowrap">
              <thead><tr className="border-b border-border">
                {['Receipt','Date','Customer','Payment','Revenue','COGS','Gross Profit','Margin','Cashier',''].map(h=><th key={h} className="text-left py-2 px-3 font-semibold text-muted-foreground">{h}</th>)}
              </tr></thead>
              <tbody>{sales.map(s=>{
                const margin = s.total_amount > 0 ? ((s.profit_amount||0)/s.total_amount*100).toFixed(1) : '0.0';
                return (
                  <tr key={s.id} className="border-b border-border hover:bg-card">
                    <td className="py-2 px-3 font-mono text-primary font-semibold">{s.receipt_number}</td>
                    <td className="py-2 px-3 text-muted-foreground">{new Date(s.created_at).toLocaleString()}</td>
                    <td className="py-2 px-3 text-muted-foreground">{s.customer_name||'—'}</td>
                    <td className="py-2 px-3"><span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${PM_COLORS[s.payment_method]||'bg-secondary text-muted-foreground'}`}>{s.payment_method}</span></td>
                    <td className="py-2 px-3 font-semibold">{fmt(s.total_amount)}</td>
                    <td className="py-2 px-3 text-muted-foreground">{fmt(s.cogs_amount||0)}</td>
                    <td className={`py-2 px-3 font-semibold ${(s.profit_amount||0)>=0?'text-green-700':'text-red-600'}`}>{fmt(s.profit_amount||0)}</td>
                    <td className="py-2 px-3 text-muted-foreground">{margin}%</td>
                    <td className="py-2 px-3 text-muted-foreground">{s.cashier_name||'—'}</td>
                    <td className="py-2 px-3"><button onClick={()=>setViewing(s)} className="p-1 hover:bg-accent rounded text-primary"><Eye className="w-4 h-4"/></button></td>
                  </tr>
                );
              })}</tbody>
            </table>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="space-y-1 text-sm border-t pt-2">
                {[['Subtotal',viewing.subtotal],['Tax',viewing.tax_amount||0],['Discount',-(viewing.discount_amount||0)]].map(([l,v])=>(
                  <div key={String(l)} className="flex justify-between text-muted-foreground"><span>{String(l)}</span><span>{fmt(Number(v))}</span></div>
                ))}
                ...
                <div className="flex justify-between text-muted-foreground text-xs"><span>COGS</span><span>{fmt(viewing.cogs_amount||0)}</span></div>
