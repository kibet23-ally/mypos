...
const TYPE_COLORS: Record<string,string> = { info:'bg-blue-100 text-blue-700', warning:'bg-orange-100 text-orange-700', error:'bg-red-100 text-red-700', success:'bg-green-100 text-green-700', announcement:'bg-purple-100 text-purple-700' };
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2"><Bell className="w-5 h-5 text-blue-600"/><h1 className="text-xl font-bold text-slate-800">Notifications</h1>{unread>0&&<Badge className="bg-blue-100 text-blue-700">{unread} unread</Badge>}</div>
      ...
      <Card style={CARD}>
        <CardContent className="pt-4">
          ...
          : items.length===0 ? <div className="text-center py-10 text-slate-400"><Bell className="w-8 h-8 mx-auto mb-2 opacity-30"/><p>No notifications.</p></div>
          : <div className="space-y-2">{items.map(n=>(
              <div key={n.id} className={`p-3 rounded-xl border ${n.read?'border-slate-100 bg-white':'border-blue-100 bg-blue-50'}`}>
                <div className="flex items-center gap-2 mb-0.5">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${TYPE_COLORS[n.type]||'bg-slate-100 text-slate-600'}`}>{n.type}</span>
