...
const STATUS_COLORS: Record<string, string> = {
  draft: 'bg-secondary text-muted-foreground', sent: 'bg-accent text-primary',
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        {canEdit && <Button onClick={() => { setForm({supplier_id:'',supplier_name:'',expected_date:'',notes:'',discount:'0'}); setItems([{...EMPTY_ITEM}]); setOpen(true); }} className="bg-primary hover:opacity-90 text-white h-9 gap-1"><Plus className="w-4 h-4" />New PO</Button>}
      </div>
      ...
      <Card style={CARD}>
        ...
        <CardContent className="overflow-x-auto">
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground"><BookOpen className="w-10 h-10 mx-auto mb-2 opacity-40" /><p>No purchase orders yet.</p></div>
          ) : (
            <table className="w-full text-sm whitespace-nowrap">
              ...
              <tbody>{orders.map(o => (
                <tr key={o.id} className="border-b border-border hover:bg-card">
                  <td className="py-2 px-3 font-mono font-semibold text-primary">{o.order_number || o.id.slice(0,8)}</td>
                  <td className="py-2 px-3 text-foreground">{o.supplier_name||'—'}</td>
                  <td className="py-2 px-3 text-muted-foreground">{o.expected_date||'—'}</td>
                  <td className="py-2 px-3 font-semibold">{fmt(o.total)}</td>
                  <td className="py-2 px-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${STATUS_COLORS[o.po_status??'draft']??'bg-secondary text-muted-foreground'}`}>{o.po_status||'draft'}</span>
                  </td>
                  <td className="py-2 px-3 text-muted-foreground">{new Date(o.created_at).toLocaleDateString()}</td>
                  <td className="py-2 px-3">
                    <div className="flex items-center gap-1">
                      <button onClick={()=>setViewing(o)} className="p-1 hover:bg-accent rounded text-primary" title="View"><Eye className="w-4 h-4" /></button>
                      {canEdit && o.po_status === 'draft' && <button onClick={()=>updateStatus(o.id,'sent')} className="p-1 hover:bg-accent rounded text-primary" title="Send"><Send className="w-4 h-4" /></button>}
                      ...
                      <button onClick={()=>printPO(o)} className="p-1 hover:bg-secondary rounded text-muted-foreground" title="Print"><BookOpen className="w-4 h-4" /></button>
        ...
        <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-3xl max-h-[90dvh] overflow-y-auto">
          ...
          <div className="space-y-4 py-2">
                ...
                <table className="w-full text-sm whitespace-nowrap">
                  ...
                  <tbody>{items.map((it,idx)=>(
                    <tr key={idx} className="border-b border-border">
                      <td className="py-1 px-2"><Input list={`po-prod-${idx}`} className="h-8 text-sm bg-card border-border rounded-lg px-2" value={it.name} onChange={e=>setItem(idx,'name',e.target.value)}/><datalist id={`po-prod-${idx}`}>{products.map(p=><option key={p.id} value={p.name}/>)}</datalist></td>
            ...
            <div className="flex justify-end text-sm space-y-1 flex-col items-end">
              <div className="flex justify-between gap-8 w-48"><span className="text-muted-foreground">Subtotal</span><span>{fmt(subtotal)}</span></div>
              <div className="flex justify-between gap-8 w-48 items-center"><span className="text-muted-foreground">Discount</span><Input type="number" min="0" className="h-7 text-sm bg-card border-border rounded-lg px-2 w-24 text-right" value={form.discount} onChange={e=>setForm(f=>({...f,discount:e.target.value}))}/></div>
              <div className="flex justify-between gap-8 w-48"><span className="text-muted-foreground">Tax ({taxRate}%)</span><span>{fmt(taxAmt)}</span></div>
              <div className="flex justify-between gap-8 w-48 font-bold text-base border-t pt-2"><span>Total</span><span className="text-primary">{fmt(grandTotal)}</span></div>
            </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button className="bg-primary hover:opacity-90 text-white" onClick={save} disabled={saving}>{saving?'Saving…':'Create PO'}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <Dialog open={!!viewing} onOpenChange={()=>setViewing(null)}>
          <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-2xl max-h-[90dvh] overflow-y-auto">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="text-right font-bold text-base border-t pt-2 flex justify-end gap-8"><span>Total</span><span className="text-primary">{fmt(viewing.total)}</span></div>
              <div className="flex justify-end gap-2">
                {canEdit && viewing.po_status === 'draft' && <Button className="bg-primary hover:opacity-90 text-white" onClick={()=>{updateStatus(viewing.id,'sent');setViewing(null);}}><Send className="w-4 h-4 mr-1"/>Send to Supplier</Button>}
