  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        {canEdit && <Button onClick={() => { setForm({supplier_id:'',supplier_name:'',payment_method:'cash',purchase_date:new Date().toISOString().slice(0,10),reference:'',notes:'',discount:'0',amount_paid:'',status:'received'}); setItems([{...EMPTY_ITEM}]); setOpen(true); }} className="bg-blue-600 hover:bg-blue-700 text-white h-9 gap-1"><Plus className="w-4 h-4" />New Purchase</Button>}
      </div>
      ...
      <Card style={CARD}>
        ...
        <CardContent className="overflow-x-auto">
          {loading ? (
            ...
            <div className="text-center py-12 text-slate-400"><ShoppingBag className="w-10 h-10 mx-auto mb-2 opacity-40" /><p>No purchases yet.</p></div>
          ) : (
            <table className="w-full text-sm whitespace-nowrap">
              ...
              <tbody>{purchases.map(p => (
                <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50">
                  <td className="py-2 px-3 font-mono font-semibold text-blue-700">{p.purchase_number}</td>
                  <td className="py-2 px-3 text-slate-700">{p.supplier_name || '—'}</td>
                  ...
                  <td className="py-2 px-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${STATUS_COLORS[p.status]??'bg-slate-100 text-slate-600'}`}>{p.status}</span>
                  </td>
                  <td className="py-2 px-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => setViewing(p)} className="p-1 hover:bg-blue-50 rounded text-blue-600" title="View"><Eye className="w-4 h-4" /></button>
          ...
          <div className="space-y-4 py-2">
                  ...
                  <tbody>{items.map((it,idx)=>(
                    <tr key={idx} className="border-b border-slate-100">
                      <td className="py-1 px-2">
                        <Input list={`pur-prod-${idx}`} className="h-8 text-sm bg-slate-50 border-slate-200 rounded-lg px-2" value={it.name}
            ...
            <div className="flex gap-4 text-sm items-end justify-end flex-wrap">
              <div className="space-y-1 text-right min-w-[160px]">
                <div className="flex justify-between gap-8"><span className="text-slate-500">Subtotal</span><span>{fmt(subtotal)}</span></div>
                <div className="flex justify-between gap-8 items-center"><span className="text-slate-500">Discount</span><Input type="number" min="0" className="h-7 text-sm bg-slate-50 border-slate-200 rounded-lg px-2 w-24 text-right" value={form.discount} onChange={e=>setForm(f=>({...f,discount:e.target.value}))}/></div>
                <div className="flex justify-between gap-8"><span className="text-slate-500">Tax ({taxRate}%)</span><span>{fmt(taxAmt)}</span></div>
                <div className="flex justify-between gap-8 font-bold text-base border-t pt-2"><span>Total</span><span className="text-blue-700">{fmt(grandTotal)}</span></div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={save} disabled={saving}>{saving?'Saving…':'Save Purchase'}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
