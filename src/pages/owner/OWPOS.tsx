...
import { cacheProducts, getCachedProducts, queuePendingSale, getPendingSaleCount, syncPendingSales } from '@/lib/offlineDb';
...
const PAYMENT_METHODS = [
  ...
  // 'mobile' was never a valid value in the payment_method enum (cash, mpesa,
  // card, bank_transfer, other) — confirmed via the offline sync error. The
  // online insert path uses this same value/column, so it's very likely
  // affected too, though that hasn't been separately confirmed with its own
  // failed-insert test. Renamed to match the real enum exactly either way.
...
export default function OWPOS() {
  ...
  const { format: formatAmt, symbol: currSymbol } = useCurrency();
  ...
  // Tax rate is sourced ONLY from tenants.tax_rate via Supabase — no hardcoded fallback.
  const taxRate = appUser?.tenant?.tax_rate;
  const hasTaxRate = taxRate !== undefined && taxRate !== null;
  ...
  const loadProducts = useCallback(async () => {
    ...
    if (!isOnline) {
      const cached = await getCachedProducts(appUser.tenant_id);
      setProducts(cached);
      return;
    }
    ...
    if (error) {
      console.error('[OWPOS] Failed to load products, falling back to cache:', error);
      const cached = await getCachedProducts(appUser.tenant_id);
      setProducts(cached);
      return;
    }
    ...
    setProducts(list);
    cacheProducts(appUser.tenant_id, list).catch(err => console.error('[OWPOS] Failed to cache products:', err));
  }, [appUser?.tenant_id, isOnline]);

  useEffect(() => { loadProducts(); refreshPendingCount(); }, [loadProducts, refreshPendingCount]);

  // Sync pending offline sales whenever we're online and there's something
  // queued — both on mount (covers "was offline, reloaded the page while
  // already back online") and the instant the 'online' event fires.
  useEffect(() => {
    if (!appUser?.tenant_id) return;

    const attemptSync = async () => {
      if (!navigator.onLine) return;
      const count = await getPendingSaleCount(appUser.tenant_id!);
      if (count === 0) return;

      toast.info(`Syncing ${count} offline sale${count === 1 ? '' : 's'}…`);
      const result = await syncPendingSales(appUser.tenant_id!, supabase);
      await refreshPendingCount();

      if (result.synced > 0) {
        toast.success(`Synced ${result.synced} offline sale${result.synced === 1 ? '' : 's'}`);
        loadProducts(); // stock just changed server-side, refresh the grid
      }
      if (result.failed > 0) {
        const firstError = result.errors[0]?.message ?? 'unknown error';
        toast.error(`Sync failed: ${firstError}`, { duration: 15000 });
        console.error('[OWPOS] Sync errors:', result.errors);
      }
    };
  ...
  }, [appUser?.tenant_id, loadProducts, refreshPendingCount]);
  ...
  const filtered = products
    ...
    .map(p => ({
      ...p,
      score: Math.max(
        fuzzyScore(p.name, search),
        fuzzyScore(p.sku, search),
        fuzzyScore(p.category, search)
      ),
    }))
    .filter(p => p.score > 0)
  ...
  const tax = hasTaxRate ? subtotal * (taxRate! / 100) : 0;
  ...
  const checkout = async () => {
    ...
    if (!hasTaxRate) { toast.error('Tax rate is not configured for this business. Set it in Settings before taking sales.'); return; }
    ...
    console.log('[OWPOS] Checkout using tax_rate:', taxRate, '%', '— tax amount:', tax.toFixed(2), '— online:', isOnline);
    setProcessing(true);
    ...
    if (!isOnline) {
      // Offline: queue locally, using the SAME real-column shape the online
      // branch below inserts with. `items` is kept separately for the
      // receipt/sale_items step at sync time — not sent to `sales` directly.
      ...
      // Only real columns from here on — confirmed against the live schema.
      // No `items`, no `transaction_id` — those columns do not exist on `sales`.
      ...
      if (error) {
        console.error('[OWPOS] Sale insert FAILED — nothing was saved:', error);
        toast.error(`Sale failed to save: ${error.message}`);
        setProcessing(false);
        return; // Do NOT show a receipt or clear the cart — the sale did not happen.
      }
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-3 overflow-hidden">
        <div>
          <div className="flex items-center gap-2 mb-3">
            <h2 className="text-lg font-bold text-slate-900 text-balance">POS — Sales</h2>
            {!isOnline && (
        ...
        <div className="flex gap-2 flex-wrap">
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${activeCategory === cat ? 'text-white' : 'text-slate-500 border border-slate-200 hover:text-white hover:bg-slate-50'}`}
              style={activeCategory === cat ? { background: '#2563EB' } : undefined}>
              {cat}
            </button>
          ))}
        </div>
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 text-center">
              <p className="text-sm text-slate-400">
                {products.length === 0 && !isOnline ? 'No cached products — connect once to load your catalogue' : 'No products found'}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 pb-2">
              {filtered.map(p => (
                <button key={p.id} onClick={() => addToCart(p)}
                  className="rounded-xl p-3 border text-left hover:border-blue-300 transition-all pos-btn-active group"
                  style={{ background: '#ffffff', borderColor: '#E2E8F0' }}>
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-2"
                    style={{ background: '#EFF6FF' }}>
                    <Zap className="w-4 h-4 text-blue-500" />
                  </div>
                  <p className="text-xs font-semibold text-slate-900 truncate text-balance">{p.name}</p>
                  <p className="text-xs text-slate-400 mt-0.5">{p.category}</p>
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-sm font-bold text-blue-500">{formatAmt(p.price)}</span>
      ...
      <div className="w-full lg:w-80 shrink-0 flex flex-col rounded-2xl border overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            {cart.length > 0 && (
              <Badge className="text-xs h-5 px-1.5" style={{ background: '#2563EB', color: '#0F172A' }}>{cart.length}</Badge>
            )}
          </div>
          {cart.length > 0 && (
            <button onClick={() => setCart([])} className="text-xs text-slate-400 hover:text-red-500 transition-colors">Clear</button>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-4 py-3">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-10">
              <ShoppingCart className="w-8 h-8 text-[#334155] mb-2" />
              ...
              {cart.map(item => (
                <div key={item.id} className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400">{formatAmt(item.price)} each</p>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button onClick={() => updateQty(item.id, -1)}
                      className="w-6 h-6 rounded-lg flex items-center justify-center bg-slate-50 hover:bg-white/10 transition-colors">
                    ...
                    <button onClick={() => removeFromCart(item.id)}
                      className="w-6 h-6 rounded-lg flex items-center justify-center hover:bg-red-500/10 transition-colors ml-1">
        ...
        <div className="border-t border-slate-100 px-4 py-4 space-y-3">
          ...
          {/* Cash tendered / change due — slides in when cash is selected */}
          {payMethod === 'cash' && (
